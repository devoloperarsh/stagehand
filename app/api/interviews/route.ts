...
export async function POST(req: Request) {
  ...
  // Sign a long-lived URL the worker can fetch.
  const admin = createAdminClient();
  const { data: signed, error: signErr } = await admin.storage
    .from("interviews")
    .createSignedUrl(parsed.data.storage_path, 60 * 60 * 6);
  if (signErr || !signed) {
    return NextResponse.json({ error: signErr?.message ?? "Sign url failed" }, { status: 500 });
  }
  ...
  if (profile.subscription_tier === "free") {
    ...
    await admin
      .from("users")
      .update({ sprint_analyses_remaining: Math.max(0, profile.sprint_analyses_remaining - 1) })
      .eq("id", user.id);
  }
