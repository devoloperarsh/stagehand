  ...
  try {
    if (!process.env.STRIPE_SECRET_KEY) {
      return NextResponse.json(
        {
          error:
            "Stripe is not configured for this environment. Set STRIPE_SECRET_KEY and the price IDs in .env.local.",
        },
        { status: 503 },
      );
    }
    ...
    if (!priceId) {
      return NextResponse.json(
        { error: `Missing Stripe price ID for "${parsed.data.plan}". Set the env var.` },
        { status: 503 },
      );
    }
    ...
    console.error("[/api/billing/stripe/checkout]", err);
    return NextResponse.json(
